# Cozy Tavern — handoff for the next session (state at m241-001)

Repo: https://github.com/brucestarkallen/Cozy-Tavern- (main). Every commit is tested first.
Full history of every law and fix: AGENTS.md (M1 … M241). SPEC.md holds the founding design.

## Run the tests before any commit (all three; all must be green)
- `node tests/harness/run.mjs` — 481 checks on the engines, assembler, workers, laws.
- `cd tests/dom && node run.mjs` — the walk: 37 scenarios of the real app in jsdom (every button,
