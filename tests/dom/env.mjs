...
export function makeHouse() {
  ...
  const fetchImpl = async (url, opts = {}) => {
    ...
    const isWorker = /keep the ledger|world beyond the page|character scribe|memory keeper|second reader|continuity reader|mend a story|narrative-state tracker|audit one record line|auditor of the ledger|referee|cast sheet|Answer with JSON ONLY|JSON ONLY/i.test(sys) && !/You are telling a story/.test(sys);
    n += 1;
